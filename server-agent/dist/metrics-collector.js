...
export class MetricsCollector {
     ...
     */
    async collect() {
        const [cpuUsagePercent, memoryUsageMb, diskUsageGb, podMetrics] = await Promise.all([
            this.getCpuUsage(),
            this.getMemoryUsage(),
            this.getDiskUsage(),
            this.getPodMetrics(),
    ...
    async getDiskUsage() {
        try {
            ...
            const { stdout } = await execAsync("df -Pk 2>/dev/null | awk '$1 ~ /^\\/dev\\// {fs[$1]=$3} END {total=0; for (f in fs) total+=fs[f]; print total}'");
    ...
    async getPodMetrics() {
        try {
            ...
            const { stdout: containerList } = await execAsync("docker ps --filter status=running --format '{{.ID}}'");
            const containerIds = containerList.trim().split("\n").filter(Boolean);
            if (containerIds.length === 0) {
                return [];
            }
            // Get stats for all containers in one shot (no-stream for single snapshot)
            const { stdout: statsOutput } = await execAsync(`docker stats --no-stream --format '{{.Container}}|{{.CPUPerc}}|{{.MemUsage}}|{{.NetIO}}' ${containerIds.join(" ")}`);
            const podMetrics = [];
            for (const line of statsOutput.trim().split("\n")) {
                ...
                // Parse CPU: "15.23%" -> 15
                const cpuUsagePercent = Math.round(parseFloat(cpuPerc.replace("%", "")) || 0);
                ...
                const netMatch = netIO.match(/([0-9.]+)(B|kB|MB|GB)\s*\/\s*([0-9.]+)(B|kB|MB|GB)/);
                ...
                const { stdout: nameOutput } = await execAsync(`docker inspect ${containerId} --format '{{.Name}}'`);
     ...
     */
    async getContainerDiskUsage(containerId) {
        try {
            const { stdout } = await execAsync(`docker exec ${containerId} df -k / | tail -1 | awk '{print $3}'`);
            const usedKb = parseInt(stdout.trim(), 10);
