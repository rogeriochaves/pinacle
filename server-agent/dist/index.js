...
export class ServerAgent {
    ...
    async registerServer() {
        ...
        try {
            const response = await fetch(`${this.config.apiUrl}/api/trpc/servers.registerServer`, {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to register server: ${response.status} ${errorText}`);
            }
    ...
    async sendHeartbeatAndMetrics() {
        ...
        try {
            ...
            // Send metrics
            const metricsResponse = await fetch(`${this.config.apiUrl}/api/trpc/servers.reportMetrics`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "x-api-key": this.config.apiKey,
            ...
            if (!metricsResponse.ok) {
                ...
                throw new Error(`Metrics report failed: ${metricsResponse.status}\n${errorText.replace("\\n", "\n")}`);
            }
            console.log(`💓 Heartbeat sent | CPU: ${metrics.cpuUsagePercent}% | Memory: ${metrics.memoryUsageMb}MB | Pods: ${metrics.activePodsCount}`);
            if (metrics.podMetrics.length > 0) {
                console.log(`📊 Pod metrics: ${metrics.podMetrics.map((pm) => `${pm.podId.substring(0, 8)}:${pm.cpuUsagePercent}%`).join(", ")}`);
            }
        }
        catch (error) {
    ...
    async getServerInfo() {
        ...
        try {
            ...
            const { stdout } = await execAsync("df -Pk 2>/dev/null | awk '$1 ~ /^\\/dev\\// {fs[$1]=$2} END {total=0; for (f in fs) total+=fs[f]; print total}'");
    ...
    saveServerConfig() {
        try {
            writeFileSync(CONFIG_FILE, JSON.stringify({ serverId: this.serverId }, null, 2), "utf-8");
